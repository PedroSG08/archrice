...
- respalda los archivos que va a sobrescribir;
...
- no cambia la sesión de inicio ni modifica perfiles de aplicaciones.

RECOMENDACIONES
===============
- Revisa el hash del archivo .tar.zst si te lo proporcionaron.
...
EOF
  ...
  if command -v zstd >/dev/null 2>&1; then
    archive="${bundle_root}.tar.zst"
    tar --zstd -cf "$archive" -C "$(dirname "$bundle_root")" "$stage_name"
  else
    archive="${bundle_root}.tar.gz"
    tar -czf "$archive" -C "$(dirname "$bundle_root")" "$stage_name"
  fi
...
backup_destination() {
  local backup=$1 relative source destination backup_copy
...
apply_staged_files() {
  ...
  cp -a -- "$STAGE_DIR/wallpapers/." "$WALLPAPER_DIR/"
}

restore_backup() {
  local backup=$1 relative destination
  ...
  if [[ -d "$backup/fondos-originales" ]]; then
    rm -rf -- "$WALLPAPER_DIR"
...
install_bundle() {
  ...
  apply_staged_files
  ok "Archivos de configuración aplicados."
...
main() {
  ...
  case "$command_name" in
    ...
    -h|--help|help)
      usage
      ;;
    --yes|--dry-run|--skip-packages|--skip-aur|--no-activate|--backup-root)
